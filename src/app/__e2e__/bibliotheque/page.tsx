...
async function e2eMoveAction(): Promise<{ status: "confirmed" }> {
  "use server";
  return { status: "confirmed" };
}

async function e2eSelectionMoveAction(_state: { status: string }, formData: FormData): Promise<{ status: "confirmed"; commandId: string }> {
  "use server";
  return { status: "confirmed", commandId: String(formData.get("commandId") ?? "") };
}

async function e2eSelectionUndoAction(): Promise<{ status: "confirmed" }> {
  "use server";
  return { status: "confirmed" };
}
  ...
  return (
    <main className="welcome-shell">
      <section className="welcome-card auth-card library-card" aria-labelledby="library-e2e-title">
        ...
        <p className="intro">Une bibliothèque de test, parcourue par statut, sans donnée personnelle.</p>
