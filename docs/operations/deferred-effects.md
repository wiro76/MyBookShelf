...
5. **Échecs journalisés côté base** : `select * from deferred.effect_failures where message_id = '<uuid>' order by occurred_at desc;` donne le détail (`error_code`, `error_message`, `read_ct`) de chaque tentative échouée pour un message donné — table alimentée dans une transaction séparée du `ROLLBACK` métier, donc jamais perdue.
